...
const UserDetails: React.FC = () => {
  ...
  const handleDelete = async (userId: number) => {
    try {
      // Assuming there is an API endpoint for deleting a user by ID
      await httpClient.delete(`//localhost:5000/userDetails/${userId}`);
      // After successful deletion, update the users state to reflect the changes
      setUsers((prevUsers) => prevUsers.filter((user) => user.employee_id !== userId));
    } catch (error) {
      console.error('Error deleting user:', error);
    }
  };

  const handleModify = (employeeId: string) => {
    navigate(`/modify-user/${employeeId}`);
  };
        ...
        <tbody>
          {filteredUsers.map((user, index) => (
            <tr key={index}>
              ...
              <td>
                <button className='delete' style={{ backgroundColor: 'white' }} onClick={() => handleDelete(user.employee_id)}>❌</button>
              </td>
              <td>
                <button className='modify' style={{ backgroundColor: 'white' }} onClick={() => handleModify(user.employee_id)}>📝</button>
              </td>
            </tr>
          ))}
        </tbody>
